...
unsigned char *frame;
{
  ...
  for (j=0; j<mb_height2; j++)
  {
    ...
    for (i=0; i<mb_width; i++)
    {
      ...
      if (mb_type & MB_INTRA)
      {
        for (comp=0; comp<block_count; comp++)
          quant_intra(blocks[k*block_count+comp],blocks[k*block_count+comp],
                      dc_prec,intra_q,mbinfo_[k].mquant);
        mbinfo_[k].cbp = cbp = (1<<block_count) - 1;
      }
      else
      {
        cbp = 0;
      ...
      if (cbp && prev_mquant!=mbinfo_[k].mquant)
        mb_type|= MB_QUANT;
      ...
      if (i!=0 && i!=mb_width-1 && !cbp)
      {
        ...
        if (pict_type==B_TYPE && pict_struct==FRAME_PICTURE
            && mbinfo_[k].motion_type==MC_FRAME
            && ((mbinfo_[k-1].mb_type^mb_type)&(MB_FORWARD|MB_BACKWARD))==0
            && (!(mb_type&MB_FORWARD) ||
                (PMV[0][0][0]==mbinfo_[k].MV[0][0][0] &&
                 PMV[0][0][1]==mbinfo_[k].MV[0][0][1]))
            && (!(mb_type&MB_BACKWARD) ||
                (PMV[0][1][0]==mbinfo_[k].MV[0][1][0] &&
                 PMV[0][1][1]==mbinfo_[k].MV[0][1][1])))
        {
        ...
        if (pict_type==B_TYPE && pict_struct!=FRAME_PICTURE
            && mbinfo_[k].motion_type==MC_FIELD
            && ((mbinfo_[k-1].mb_type^mb_type)&(MB_FORWARD|MB_BACKWARD))==0
            && (!(mb_type&MB_FORWARD) ||
                (PMV[0][0][0]==mbinfo_[k].MV[0][0][0] &&
                 PMV[0][0][1]==mbinfo_[k].MV[0][0][1] &&
                 mbinfo_[k].mv_field_sel[0][0]==(pict_struct==BOTTOM_FIELD)))
            && (!(mb_type&MB_BACKWARD) ||
                (PMV[0][1][0]==mbinfo_[k].MV[0][1][0] &&
                 PMV[0][1][1]==mbinfo_[k].MV[0][1][1] &&
                 mbinfo_[k].mv_field_sel[0][1]==(pict_struct==BOTTOM_FIELD))))
        {
      ...
      if (mb_type & (MB_FORWARD|MB_BACKWARD) && !frame_pred_dct)
        putbits(mbinfo_[k].motion_type,2);

      if (pict_struct==FRAME_PICTURE && cbp && !frame_pred_dct)
        putbits(mbinfo_[k].dct_type,1);

      if (mb_type & MB_QUANT)
      {
        putbits(q_scale_type ? map_non_linear_mquant[mbinfo_[k].mquant]
                             : mbinfo_[k].mquant>>1,5);
        prev_mquant = mbinfo_[k].mquant;
      }

      if (mb_type & MB_FORWARD)
      {
        /* forward motion vectors, update predictors */
        putmvs(mbinfo_[k].MV,PMV,mbinfo_[k].mv_field_sel,mbinfo_[k].dmvector,0,
          mbinfo_[k].motion_type,forw_hor_f_code,forw_vert_f_code);
      }

      if (mb_type & MB_BACKWARD)
      {
        /* backward motion vectors, update predictors */
        putmvs(mbinfo_[k].MV,PMV,mbinfo_[k].mv_field_sel,mbinfo_[k].dmvector,1,
          mbinfo_[k].motion_type,back_hor_f_code,back_vert_f_code);
      }
