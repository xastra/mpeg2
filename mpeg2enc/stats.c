...
unsigned char *rec[3];
{
  ...
  fprintf(statfile,"Y: variance=%4.4g, MSE=%3.3g, PSNR=%3.3g dB, SNR=%3.3g dB\n",
...
void stats()
{
  ...
  for (j=0; j<mb_height2; j++)
  {
    for (i=0; i<mb_width; i++)
    {
      if (i==0 || mbinfo_[k].mquant!=mbinfo_[k-1].mquant)
        fprintf(statfile,"%3d",mbinfo_[k].mquant);
      else
        fprintf(statfile,"   ");
