  ...
  for (i=0; i<nframes; i++)
  {
    ...
    if (fieldpic)
    {
      ...
      motion_estimation(oldorgframe[0],neworgframe[0],
                        ...
                        neworg[0],newref[0],
                        sxf,syf,sxb,syb,mbinfo_,0,0);

      predict(oldrefframe,newrefframe,predframe,0,mbinfo_);
      dct_type_estimation(predframe[0],neworg[0],mbinfo_);
      transform(predframe,neworg,mbinfo_,blocks);

      putpict(neworg[0]);

      for (k=0; k<mb_height2*mb_width; k++)
      {
        if (mbinfo_[k].mb_type & MB_INTRA)
      ...
      itransform(predframe,newref,mbinfo_,blocks);
      ...
      motion_estimation(oldorgframe[0],neworgframe[0],
                        oldrefframe[0],newrefframe[0],
                        neworg[0],newref[0],
                        sxf,syf,sxb,syb,mbinfo_,1,ipflag);

      predict(oldrefframe,newrefframe,predframe,1,mbinfo_);
      dct_type_estimation(predframe[0],neworg[0],mbinfo_);
      transform(predframe,neworg,mbinfo_,blocks);
