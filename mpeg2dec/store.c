...
int frame;
{
  ...
  {
    /* progressive */
   // sprintf(outname,Output_Picture_Filename,frame,'f'); //yao 
	  sprintf(outname,Output_Picture_Filename,0,'f');
...
int offset,incr,height;
{
 ...
 // sprintf(tmpname,"%s.Y",outname);// yao 09/3/11
   sprintf(tmpname,"%s.yuv",outname);
   if ((outfile = open(tmpname,O_CREAT|O_APPEND |O_WRONLY|O_BINARY,0666))==-1)
   {
  ...
  //sprintf(tmpname,"%s.U",outname);//yao 09/3/11
  store_yuv1(tmpname,src[1],offset,incr,hsize,height);
fprintf(stderr,"saving %s\n",tmpname);
 // sprintf(tmpname,"%s.V",outname);//yao 09/3/11
